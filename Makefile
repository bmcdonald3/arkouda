...
ARKOUDA_SERVER_MODULES=$(shell cat $(ARKOUDA_CONFIG_FILE))
ARKOUDA_SOURCE_FILES=$(ARKOUDA_SERVER_MODULES:%=$(ARKOUDA_SOURCE_DIR)/%.chpl)

# This is the main compilation statement section
$(ARKOUDA_MAIN_MODULE): check-deps $(ARROW_O) $(ARKOUDA_SOURCES) $(ARKOUDA_MAKEFILES)
	$(CHPL) $(CHPL_DEBUG_FLAGS) $(PRINT_PASSES_FLAGS) $(REGEX_MAX_CAPTURES_FLAG) $(OPTIONAL_SERVER_FLAGS) $(CHPL_FLAGS_WITH_VERSION) $(ARKOUDA_MAIN_SOURCE) $(ARKOUDA_COMPAT_MODULES) $(ARKOUDA_SOURCE_FILES) $(ARKOUDA_SERVER_USER_MODULES) -o $@
