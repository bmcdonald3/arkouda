...
def time_ak_write(N_per_locale, trials, dtype, path, seed, parquet, numfiles):
    print(">>> arkouda {} write".format(dtype))
    cfg = ak.get_config()
    N = N_per_locale * cfg["numLocales"]
    print("numLocales = {}, N = {:,}, numFiles {}".format(cfg["numLocales"], N, numfiles))
    if dtype == 'int64':
    ...
    avgwrite = sum(writetimes) / len(writetimes)
...
def time_ak_read(N_per_locale, trials, dtype, path, seed, parquet, numfiles):
    ...
    for i in range(trials):
        start = time.time()
        a = ak.load(path+'*') if not parquet else ak.read_parquet(path+'*')
        end = time.time()
...
def check_correctness(dtype, path, seed, parquet):
    N = 10**4
    if dtype == 'int64':
        a = ak.randint(0, 2**32, N, seed=seed)
    elif dtype == 'float64':
...
if __name__ == "__main__":
    ...
    if args.only_write:
        time_ak_write(args.size, args.trials, args.dtype, args.path, args.seed, args.parquet, args.files_per_loc)
    elif args.only_read:
        time_ak_read(args.size, args.trials, args.dtype, args.path, args.seed, args.parquet, args.files_per_loc)
