module LisExprInterp
{
    ...
    */
    proc eval(ast: BGenListValue, env: borrowed Env, st, ref otherT, ref lookupT): GenValue throws {
      otherT.start();
        select (ast.lvt) {
            when (LVT.Sym) {
            ...
            when (LVT.Lst) {
                ...
                select (op) {
                    when "+"   {checkEqLstSize(lst,3);
                      otherT.stop();
                      return eval(lst[1], env, st, otherT, lookupT) + eval(lst[2], env, st, otherT, lookupT);}
                    when "-"   {checkEqLstSize(lst,3);
                      otherT.stop();
                      return eval(lst[1], env, st, otherT, lookupT) - eval(lst[2], env, st, otherT, lookupT);}
                    when "*"   {checkEqLstSize(lst,3);
                    ...
                    when ">="  {checkEqLstSize(lst,3);
                      otherT.stop();
                      return eval(lst[1], env, st, otherT, lookupT) >= eval(lst[2], env, st, otherT, lookupT);}
                    when "or"  {checkEqLstSize(lst,3);
                      otherT.stop();
                      return or(eval(lst[1], env, st, otherT, lookupT), eval(lst[2], env, st, otherT, lookupT));}
                    when "and" {checkEqLstSize(lst,3);
                      otherT.stop();
                      return and(eval(lst[1], env, st, otherT, lookupT), eval(lst[2], env, st, otherT, lookupT));}
                    when "not" {checkEqLstSize(lst,2);
                      otherT.stop();
                      return not(eval(lst[1], env, st, otherT, lookupT));}
                    when ":=" {
                        ...
                        otherT.stop();
                        var gv = env.addEntry(name, eval(lst[2],env, st, otherT, lookupT));
                    ...
                    when "lookup_and_index_float64" {
                      ...
                      lookupT.start();
                        ...
                        lookupT.stop();
                        var i = eval(lst[2],env,st,otherT,lookupT).toValue(int).v;
                    ...
                    when "if" {
                        ...
                        otherT.stop();
                        if isTrue(eval(lst[1], env, st, otherT, lookupT)) {return eval(lst[2], env, st, otherT, lookupT);} else {return eval(lst[3], env, st, otherT, lookupT);}
                    }
                    when "begin" {
                      checkGEqLstSize(lst, 1);
                      ...
                      for i in 1..#lst.size-1 {
                        eval(lst[i], env, st, otherT, lookupT);
                      }
                      // eval the return expression
                      return eval(lst[lst.size-1], env, st, otherT, lookupT);
                    }
                    when "return" { // for now, just eval the next line, in time, might want to coerce return value
                      otherT.stop();
                        return eval(lst[1], env, st, otherT, lookupT);
                    }
                    otherwise {
