...
module RadixSortLSD
{
    ...
    private proc radixSortLSDCore(a:[?aD] ?t, nBits, negs, comparator) {
        ...
        for rshift in {0..#nBits by bitsPerDigit} {
            ...
            prev = calcPosAndPermuteT.elapsed();
    ...
    proc radixSortLSD_ranks(a:[?aD] ?t, checkSorted: bool = true, print: bool = false): [aD] int {
        checkSortedT.clear();
        arrayCreateT.clear();
        countDigitsT.clear();
        globalCountsT.clear();
        calcPosAndPermuteT.clear();
        arrayCopyT.clear();
        ranksAssignT.clear();
        checkSortedT.start();
        if (checkSorted && isSorted(a)) {
